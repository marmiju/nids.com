...
interface Props {
  ...
  onChngeError: (error: string | null) => void;
  output: string;
  ...
  return (
    <div>
      <div className="pt-2 rounded mb-4 grid grid-cols-1 lg:grid-cols-2 gap-2">
        {/* Problem statement */}
        <div className="bg-secondary shadow-sm p-2 rounded-2xl ">
          <div className="flex justify-between">
            <h2 className="text-sm font-light text-center border rounded-full text-text border-gray-200 shadow px-2">
              Id: {problem.id}
            </h2>
            {userData && userData.value.includes(problem.id) && (
              <p className="text-sm font-light text-center border rounded-full border-green-600  bg-secondary text-text  shadow-green-500 px-2">
                ⨀ Solved
              </p>
            )}
          </div>
          <h2 className="text-xl text-primary font-semibold">{problem.title}</h2>

          <p className="text-text bg-secondary mr-2 p-2">
            {problem.description}
          </p>
          <div className={` mr-2 text-text my-10 space-y-4 `}>
            Input:
            <pre className="bg-text/10 p-2 mb-2 rounded text-text">
              3 <br></br>
              {
                problem.input &&
                problem.input
            ...
            Expected Output:
            <pre
              className={`bg-text/10 p-2 mb-2  rounded ${isAccepted === null
                ? "text-text"
                : isAccepted
                  ? "text-green-600"
                  : "text-red-600"
            ...
            {output ? (
              ...
              >
                Your Output:
                <pre
                  className={`bg-text/10 p-2 rounded whitespace-pre-wrap ${isAccepted === null
                    ? " text-text"
                    : isAccepted
                      ? "text-green-500"
                      : "text-red-500"
