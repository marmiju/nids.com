    ...
    return (
        <div className="max-w-[1280px] mx-auto p-4 ">
            ...
            {contestData.contests.map((contest) => {
                return <Link key={contest.id} href={`event/${contest.id}`}><Contest end_time={contest.end_time} created_at={contest.created_at} description={contest.description} hosted_by={contest.hosted_by} id={contest.id} title={contest.title} problems={contest.problems} /></Link>
            }
            )}
        </div>
    );
